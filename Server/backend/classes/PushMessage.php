...
class PushMessage {
    ...
    public function sendMessage($title, $message, $level = 0){
        ...
        $bdd = getBDD();
        $req = $bdd->prepare('SELECT at_gcm.id FROM at_gcm JOIN at_devices ON at_gcm.mac = at_devices.mac JOIN at_users ON at_devices.username = at_users.nom WHERE at_users.type <= :type');
        $req->execute(array('type' => $level));
        while ($data = $req->fetch()) {
            array_push($devices, $data['id']);
        }
        $req->closeCursor();
    ...
    public function sendMessageAll($title, $message){
        ...
        $gcpm = new GCMPushMessage($this->apiKey);
        $bdd = getBDD();
        $req = $bdd->query('SELECT id FROM at_gcm');
        while ($data = $req->fetch()) {
            array_push($devices, $data['id']);
        }
        $req->closeCursor();
