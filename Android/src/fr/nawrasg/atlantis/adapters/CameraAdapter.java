...
public class CameraAdapter extends RecyclerView.Adapter<CameraAdapter.CameraViewHolder> {
	...
	public void onBindViewHolder(final CameraViewHolder holder, int position) {
		...
		Picasso.with(mContext).load(App.getUri(mContext, App.Images) + "&type=camera&id=" + nCamera.getID()).into(holder.image);
		String nAlias = "";
		...
		holder.image.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				Picasso.with(mContext).load(App.getUri(mContext, App.Images) + "&type=camera&id=" + nCamera.getID()).memoryPolicy(MemoryPolicy.NO_CACHE).into(holder.image);
			}
		});
	}
